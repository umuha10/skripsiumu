		...
		<div class="row mt-3 ">
			...
			<div class="modal fade" id="insert" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
				<div class="modal-dialog" role="document">
					<form action="<?= site_url('kriteria/proses/subkriteria') ?>" method="post">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLabel">Tambah Parameter Kriteria</h5>
								...
								<div class="row">
									<div class="col-sm-12">
										...
										<select name="id_kriteria" class="form-control" id="id_kriteria" required>
											<?php foreach ($kriteria->result() as $key => $data) : ?>
												<option value="<?= $data->id ?>"><?= $data->kriteria ?></option>
											<?php endforeach; ?>
										</select>
										<label> Parameter Kriteria *</label>
										<input type="text" name="subkriteria" class="form-control" required>

										<label for="bobot">Bobot</label>
										<input type="number" min="0" max="5" class="form-control" name="bobot" required>
									</div>
								</div>
							</div>
